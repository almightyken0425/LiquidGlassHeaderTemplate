...
- **頂層 Product git：**
    - 即本 repo
    - 目前 `no1_product_initiation/`、`no2_product_planning/` 皆為 placeholder
    - 模板庫本質不需要完整決策框架上游產出，保留結構以備日後擴充
- **Module Design git：**
    - `no4_product_designs/` 容器目錄已預留，目前無 Design git
    - 模板庫的視覺由 Impl 直接承載
- **Module Spec git：**
...
2026-07-21 層重編號：spec 改回 `no3_product_specs/`、design 改為 `no4_product_designs/`、impl 改為 `no5_product_development/`；`no5_project_management/` 搬出產品，內容移至 ai-company 根 git 的 `project/LiquidGlassHeaderTemplate/`。
